...
const uncomplicated_player_queue_1 = __importDefault(require("./uncomplicated-player-queue"));
/**
 * Player mechanism:
...
const UncomplicatedPlayer = (() => {
    ...
    let instance;
    const init = (latencyMode = 'playback', __logging = false, __logger = () => { }) => {
        ///////////////////////////////
        ///////////////////////////////
        ////// private variables //////
        ...
        // setting up queue
        let ucpQueue = new uncomplicated_player_queue_1.default();
        ...
        // setting up players
        let playersCount = 2 * defaultConfig.prefetchSize + 1;
         ...
         */
        const exponentialGainTransition = (gainNode, targetGain, duration) => {
            if (duration === 0) {
            ...
            gainNode.gain.setTargetAtTime(targetGain, audioContext.currentTime, timeConst);
            return new Promise(resolvePromise => {
        ...
        const adjustGain = () => {
            if (config.smoothGainTransition) {
                ...
                exponentialGainTransition(players[_currentPlayer].gainNode, config.globalGain, config.smoothGainTransitionDuration);
            }
            else {
                players.forEach(player => (player.gainNode.gain.value = config.globalGain));
            }
        };
        // pauses the player according to the configs
        const playerPause = (player, fade) => {
            player.state++;
            let playerState = player.state;
            exponentialGainTransition(player.crossfadeNode, 0, fade ? config.crossfadeDuration : 0)
                .then(() => {
                // confirm if player state is not changed meanwhile
                if (player.state === playerState) {
                    player.sourceNode.mediaElement.pause();
                    player.crossfadeNode.gain.cancelAndHoldAtTime(audioContext.currentTime);
                    player.crossfadeNode.gain.value = 0;
                }
            })
                .catch(() => makeLog(`Error at pausing player ${_currentPlayer}`));
        };
        // plays the player according to the configs
        const playerPlay = (player, fade) => {
            player.state++;
            let playerState = player.state;
            makeLog('playerPlay - src -', player.sourceNode.mediaElement.src);
            player.sourceNode.mediaElement.play();
            exponentialGainTransition(player.crossfadeNode, config.globalGain, fade ? config.crossfadeDuration : 0)
                .then(() => {
                // confirm if player state is not changed meanwhile
                if (player.state === playerState) {
                    player.crossfadeNode.gain.cancelAndHoldAtTime(audioContext.currentTime);
                ...
                .catch(() => makeLog(`Error at playing player ${_currentPlayer}`));
        };
        // switch from player1 to player2 while playing
        const switchPlayers = (oldIndex, newIndex, fade) => {
        ...
        const getNextPlayers = () => {
            ...
            for (let i = 1; i <= config.prefetchSize; ++i) {
                indexes.push(_currentPlayer + i < playersCount
                    ? _currentPlayer + i
        ...
        const getPrevPlayers = () => {
            let indexes = [];
            for (let i = 1; i <= config.prefetchSize; ++i) {
                indexes.push(_currentPlayer - i >= 0
                    ? _currentPlayer - i
                    : _currentPlayer - i + playersCount);
            }
            indexes.reverse();
        ...
        const updatePrefetch = () => {
            ...
            let prevPlayers = getPrevPlayers();
            ...
            nextPlayers.forEach((playerIndex, i) => {
                ...
                else if (players[playerIndex].sourceNode.mediaElement.src !==
                    nextSeek[i].src.toString()) {
                    makeLog('updatePrefetch - next - ', playerIndex, players[playerIndex].sourceNode.mediaElement.src, nextSeek[i].src.toString());
                    players[playerIndex].sourceNode.mediaElement.src =
                        nextSeek[i].src.toString();
                }
            });
            prevPlayers.forEach((playerIndex, i) => {
                if (!prevSeek[i])
                    ...
                    makeLog('updatePrefetch - prev - ', playerIndex, players[playerIndex].sourceNode.mediaElement.src, prevSeek[i].src.toString());
                    players[playerIndex].sourceNode.mediaElement.src =
                        prevSeek[i].src.toString();
                }
            });
        ...
        const adjustPlayers = () => {
            if (config.prefetchSize === ucpQueue.seekLength)
                return;
            let diff = 2 * ucpQueue.seekLength + 1 - playersCount;
            if (diff > 0) {
                // new players added
                ...
                rightRange = rightRange.map(idx => idx >= playersCount ? playersCount : idx);
        ...
        const queueMutationCallback = (args) => {
            if (!args)
                return;
            let oldCurrentIndex = _currentPlayer;
            ...
            switch (args[0]) {
                ...
                case 'setDefaultSeekLength': {
                    ...
                    adjustPlayers();
                    makeLog('Mutation callback - set seekLength - num or default');
                    break;
                }
                case 'shuffle':
        ...
        return {
            ...
            play: () => {
                ...
                if (audioContext.state === 'suspended') {
                    audioContext.resume().then(() => {
                        playerPlay(players[_currentPlayer], config.crossfade && config.crossfadeManualSwitch);
            ...
            pause: () => {
                ...
                playerPause(players[_currentPlayer], config.crossfade && config.crossfadeManualSwitch);
            ...
            set volume(volume) {
                if (volume < 0.0)
                    config.globalGain = 0.0;
                else if (volume > 1.0 && !config.allowGainBoost)
                    config.globalGain = 1.0;
                else
                    config.globalGain = volume;
                adjustGain();
                makeLog('set volume: ' +
                    'input: ' +
                    volume +
            ...
            set gainDelta(delta) {
                if (0.0 < delta && delta < 1.0)
                    config.gainDelta = delta;
                else
                    config.gainDelta = defaultConfig.gainDelta;
            ...
            set smoothGainTransition(params) {
                ...
                makeLog('set smooth gain transition:', config.smoothGainTransition ? 'enabled' : 'disabled', 'duration:', config.smoothGainTransitionDuration);
            },
            ...
            set crossfade(crossfadeParams) {
                // new config = params override current config
                let params = Object.assign(Object.assign({}, this.crossfade), crossfadeParams);
                ...
                else if (params.crossfadeDuration &&
    ...
    return {
         ...
         */
        initInstance: (latencyMode = 'playback', logging = false, logger) => {
            if (instance)
                return false;
...
exports.default = UncomplicatedPlayer;
