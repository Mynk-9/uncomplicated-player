...
class UncomplicatedPlayerQueue {
    constructor() {
        ...
        this.seekSize = 3;
        this.queueMutationCallback = (args) => { };
    }
    /**
     ...
     */
    recursiveCompare(original, comp) {
        if (comp === original)
            return true;
    ...
    refreshQueue() {
        const adjustNextSeek = () => {
            let nextSeekLen = this.queue.nextSeek.length;
            ...
            // to shuffle state
                ...
                let needed = Math.min(this.seekSize - nextSeekLen, keys.length);
                if (this.shuffleQueue) {
                    for (let i = 0; i < needed; ++i) {
                        let random = Math.floor((keys.length - i) * Math.random()) + i;
        ...
        if (this.seekSize > 0 && this.queue.nextSeek.length === 0)
            return;
    ...
    push(track) {
        ...
        this.queue.next[key.toString()] = Object.assign(Object.assign({}, track), { key: key });
        this.refreshQueue();
    ...
    pushMany(tracks) {
        ...
        tracks.forEach(track => {
            let key = ++this.tracksKeyCounter;
            this.queue.next[key.toString()] = Object.assign(Object.assign({}, track), { key: key });
            tracksAdded.push(Object.assign(Object.assign({}, track), { key: key }));
        });
        this.refreshQueue();
    ...
    addNext(track) {
        let key = ++this.tracksKeyCounter;
        this.queue.nextSeek = [Object.assign(Object.assign({}, track), { key: key }), ...this.queue.nextSeek];
        this.refreshQueue();
    ...
    remove(search) {
        ...
        this.queue.next = Object.keys(this.queue.next)
            .filter((key) => {
            let predicateVal = !this.recursiveCompare(this.queue.next[key], search);
            if (!predicateVal)
                removedKeys.push(parseInt(key));
            return predicateVal;
        })
            .reduce((newNext, key) => {
        ...
        [this.queue.nextSeek, this.queue.history] = [
            this.queue.nextSeek,
            this.queue.history,
        ].map(queueComponent => queueComponent.filter(track => {
            let predicateVal = !this.recursiveCompare(track, search);
            if (!predicateVal)
                removedKeys.push(track.key);
            return predicateVal;
        }));
        /// current
    ...
    next() {
        ...
        if (keys.length === 0) {
            if (this.queue.curr)
                this.queue.history.push(this.queue.curr);
            this.queue.curr = this.queue.nextSeek[0];
            this.queue.nextSeek.splice(0, 1);
    ...
    get isNextEmpty() {
        ...
        if (this.seekSize > 0)
            return this.queue.nextSeek.length === 0;
        else
            return Object.keys(this.queue.next).length === 0;
    }
    /**
     ...
     */
    get seek() {
        let nextSeek = this.queue.nextSeek;
        let prevSeek = [];
     ...
     * if state changed.
     */
    set shuffle(shuffleEnable) {
        if (shuffleEnable === this.shuffleQueue)
            return;
     ...
     */
    set _mutationCallback(newMutationCallback) {
        this.queueMutationCallback = newMutationCallback;
    }
}
exports.default = UncomplicatedPlayerQueue;
